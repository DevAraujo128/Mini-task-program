...
import { taskManager } from "../manager/task.js";
...
export async function updateTaskMenu(taskName){
  ...
  const selected = await select({
    ...
    options: [
      { label: "Alternar nome", value: "name" },
      { label: "Alternar status", value: "status" },
  ...
  switch(selected){
    ...
    case "name": {
      ...
      taskManager.tasks.set(newTaskName, updateTaskMenu);
      taskManager.save();
      return;
    }
    case "status": {
